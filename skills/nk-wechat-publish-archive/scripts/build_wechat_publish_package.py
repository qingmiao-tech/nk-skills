...
from wechat_publish_config import (
    load_config,
    resolve_config_int,
    resolve_config_path,
...
COVER_SCRIPT_PATH = (
    SCRIPT_DIR / "build_wechat_cover_package.py"
    if (SCRIPT_DIR / "build_wechat_cover_package.py").exists()
    else REPO_ROOT / "00.系统配置" / "wechat-cover-summary-tool" / "build_wechat_cover_package.py"
)
HTML_SCRIPT_CANDIDATES = [
    SCRIPT_DIR / "convert_markdown.py",
    ...
    REPO_ROOT / ".codex" / "skills" / "nk-wechat-mdnice-blue-html" / "scripts" / "convert_markdown.py",
...
WECHAT_HEADER_TEMPLATE_PATH = resolve_optional_config_path(REPO_ROOT, PUBLISH_CONFIG, "header_template")
WECHAT_FOOTER_TEMPLATE_PATH = resolve_optional_config_path(REPO_ROOT, PUBLISH_CONFIG, "footer_template")
WECHAT_ARTICLE_LIBRARY_PATH = resolve_config_path(REPO_ROOT, PUBLISH_CONFIG, "article_library_json")
PUBLISH_INDEX_FILENAME = str(PUBLISH_CONFIG.get("publish_index_filename") or "发布索引.md")
MAX_RELATED_ARTICLES = resolve_config_int(PUBLISH_CONFIG, "related_articles_limit")
...
PUBLISH_ENV_CANDIDATES = [
    SCRIPT_DIR / "publish_wechat.env.ps1",
    ...
    REPO_ROOT / ".codex" / "skills" / "nk-wechat-mdnice-blue-html" / "scripts" / "publish_wechat.env.ps1",
...
PUBLISH_ENV_FALLBACK_CANDIDATES = [
    SCRIPT_DIR / "publish_wechat.env.ps1.example",
    SCRIPT_DIR / "publish_wechat.env.template.ps1",
...
def load_brand_header_html() -> str:
    if not WECHAT_HEADER_TEMPLATE_PATH or not WECHAT_HEADER_TEMPLATE_PATH.exists():
        return ""
...
def render_brand_footer_html(
    ...
    *,
    ...
    temp_base_path = WECHAT_FOOTER_TEMPLATE_PATH or article_path.with_name(f"{article_path.stem}-related-footer.md")
    temp_fragment_path = temp_base_path.with_suffix(f".{theme}.temp.wechat.html")
    temp_preview_path = temp_base_path.with_suffix(f".{theme}.temp.preview.html")
    result = html_module.convert_markdown_to_files(
        ...
        image_base_dir=temp_base_path.parent,
...
def load_articles_from_publish_indexes() -> list[dict[str, str]]:
    articles: list[dict[str, str]] = []
    for index_path in PUBLISH_DOC_ROOT.glob(f"*/发布/{PUBLISH_INDEX_FILENAME}"):
